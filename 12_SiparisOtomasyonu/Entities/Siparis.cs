...
namespace _12_SiparisOtomasyonu.Entities
{
    public class Siparis
    {
        ...
        public override string ToString()
        {
            if (Extralar.Count < 1)
            {
                return $"{SeciliMenu.MenuAdi} Menu x {Adet} , {Boyutu.ToString()} , Toplam {ToplamTutar.ToString()}";
            }
            else
            {
                string str = null;
                ...
                extralar = extralar.Trim(',');  //En sondaki , ü silmek icin gerekli

                return $"{SeciliMenu.MenuAdi} Menu x {Adet} , {Boyutu.ToString()} ,Extralar: {extralar}, Toplam {ToplamTutar.ToString()}";
            }
        }
